                ...
                <Grid item xs={12} sm={12} md={6} lg={6} xl={6}>
                    ...
                    <Paper variant='outlined' square={false} >
                        <div className={ styles.gridSubItem }>
                            <Grid container>
                                <Grid item xs={1} sm={1} md={1} lg={1} xl={1} className={ styles.leftGridItem }>
                                ...
                                <Grid item xs={11} sm={11} md={11} lg={11} xl={11} className={ styles.rightGridItem }>
                            ...
                            <Grid container className={ styles.gridSubContainer }>
                                <Grid item xs={12} sm={1} md={1} lg={1} xl={1} style={{ display: 'grid', placeSelf: 'center' }}>
                                    <a href={`${siteUrl}/${policy.ServerRelativeUrl}`} target='_blank'>
                                        <FileTypeIcon type={IconType.image} application={ _typeChecker(policy) } size={ ImageSize.medium }/>
                                    </a>
                                </Grid>
                                <Grid item xs={12} sm={11} md={11} lg={11} xl={11} style={{ display: 'grid', alignItems: 'center' }}>
                                    <div style={{ padding: '0 10px' }}>
                                        <a href={`${siteUrl}${policy.ServerRelativeUrl}`} target='_blank' className={ styles.fileName }>{policy.Name}</a>
                                        <div className={ styles.createdDate }>{new Date(policy.TimeCreated).toLocaleDateString('en-US')}</div>
                                    </div>
                                </Grid>
                            </Grid>
                            ))}
